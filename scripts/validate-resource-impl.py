#!/usr/bin/env python3
"""Validate a resource-implementation.yaml file against schemas/resource-implementation.schema.json.
...
def main() -> int:
    ...
    parser.add_argument("yaml_path", type=Path, help="Path to resource-implementation.yaml")
    parser.add_argument(
        "--schema",
        type=Path,
        default=Path(__file__).resolve().parent.parent
        / "schemas"
        / "resource-implementation.schema.json",
    )
    ...
    if not errors:
        print(f"OK: {args.yaml_path} is valid against resource-implementation schema.")
        return 0
